# enough for now, some improvements might be done in future

import streamlit as st
import pandas as pd

def extract_workspace_id(mongo_client, post_client):
    criteria = st.selectbox("Select Criteria:", ["Workspace Name", "Email Address", "Website"])
    input_value = st.text_input(f"Enter {criteria}:")

    if st.button("Search"):
        if input_value:
            workspace_data = fetch_workspace_data(post_client, criteria, input_value)
            if workspace_data:
                ...
                workspace_usage(workspace_ids[0], mongo_client, post_client)
...
def fetch_workspace_data(post_client, criteria, input_value):
    """
...
def workspace_usage(target_workspace_id, mongo_client, post_client):
